...
state_to_remove = """export default function Home() {
...
original_state = """export default function Home() {
  const [clusters, setClusters] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {"""

content = content.replace(state_to_remove, original_state)
...
content = content.replace(ui_to_remove, original_ui)
